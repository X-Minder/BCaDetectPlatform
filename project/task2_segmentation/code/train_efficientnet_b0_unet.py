# %% [markdown]
# # EfficientNet-B0 UNet 膀胱镜图像分割训练脚本
# 本脚本实现了基于EfficientNet-B0 UNet的医学图像分割模型训练。
# 主要功能：
# 1. 数据加载和预处理
# 2. 模型训练和验证
# 3. 指标计算和可视化
...
class ModelConfig:
    # 基础配置
    MODEL_NAME: str = "EfficientNetB0UNet" # 模型名称，用于保存和加载
    RANDOM_SEED: int = 42
    ...
    IN_CHANNELS: int = 3
    OUT_CHANNELS: int = 1
    ENCODER_PRETRAINED: bool = True # 是否使用预训练编码器的权重
    EFFICIENTNET_BACKBONE_NAME: str = "efficientnet_b0" # EfficientNet骨干名称
    ...
    # 保存配置
    SAVE_DIR: str = "checkpoints_efficientnet_b0_unet"
    
    # 可视化配置
    VISUALIZE_PREDICTIONS: bool = True
    NUM_VISUALIZATION_SAMPLES: int = 5

    FIGURE_DIR: str = "figures_efficientnet_b0_unet" # 保存训练曲线图的目录
...
# %% [markdown]
# ## EfficientNet-B0 UNet 模型定义

# %%
class EfficientNetB0UNet(nn.Module):
    def __init__(self, n_classes=1, pretrained_encoder=True):
        super().__init__()
        ...
        # Encoder stages (feature extraction points)
        # Channels for EfficientNet-B0 features attribute (output of each features[i]):
        # features[0]: MBConv1_3x3_skip: 32 -> 16 (output is 16, H/2)
        # features[1]: MBConv2_3x3_skip: 16 -> 24 (output is 24, H/4)
        # features[2]: MBConv2_5x5_skip: 24 -> 40 (output is 40, H/8)
        # features[3]: MBConv3_3x3_skip: 40 -> 80 (output is 80, H/16)
        # features[4]: MBConv1_5x5_skip: 80 -> 112 (output is 112, H/16)
        # features[5]: MBConv2_5x5_skip: 112 -> 192 (output is 192, H/32)
        # features[6]: MBConv1_3x3_skip: 192 -> 320 (output is 320, H/32) - Bottleneck
        # features[7]: Last stage convolutions, not typically used for U-Net skip connection directly
        
        # For U-Net skip connections, we usually tap into the outputs of blocks *before* downsampling too much or *after* a stage.
        # Let's get the features from the .features attribute and identify good skip connection points.
        # The output channels of `self.encoder.features[i]` are:
        # self.encoder.features[0] -> First conv layer, out_channels = 32, H/2
        # self.encoder.features[1] -> Block 0 (e.g., MBConv), out_channels = 16, H/2 
        # self.encoder.features[2] -> Block 1, out_channels = 24, H/4
        # self.encoder.features[3] -> Block 2, out_channels = 40, H/8
        # self.encoder.features[4] -> Block 3, out_channels = 80, H/16
        # self.encoder.features[5] -> Block 4, out_channels = 112, H/16
        # self.encoder.features[6] -> Block 5, out_channels = 192, H/32
        # self.encoder.features[7] -> Block 6, out_channels = 320, H/32 (Bottleneck before final conv layers in classifier)
        # self.encoder.features[8] -> Final conv layer in features block, out_channels = 1280, H/32

        # We will take skip connections from the output of specific layers within `self.encoder.features`
        self.skip_ch_s0 = 32  # Output of self.encoder.features[0] (initial conv, H/2)
        self.skip_ch_s1 = 16  # Output of self.encoder.features[1] (after block 0, H/2)
        self.skip_ch_s2 = 24  # Output of self.encoder.features[2] (after block 1, H/4)
        self.skip_ch_s3 = 40  # Output of self.encoder.features[3] (after block 2, H/8)
        self.skip_ch_s4 = 112 # Output of self.encoder.features[5] (after block 4, H/16)
        self.bottleneck_ch = 320 # Output of self.encoder.features[7] (after block 6, H/32)

        # Decoder
        self.upconv4 = nn.ConvTranspose2d(self.bottleneck_ch, 256, kernel_size=2, stride=2)
        self.dec4 = self._conv_block(256 + self.skip_ch_s4, 256) # Skip from features[5] output

        self.upconv3 = nn.ConvTranspose2d(256, 128, kernel_size=2, stride=2)
        self.dec3 = self._conv_block(128 + self.skip_ch_s3, 128) # Skip from features[3] output

        self.upconv2 = nn.ConvTranspose2d(128, 64, kernel_size=2, stride=2)
        self.dec2 = self._conv_block(64 + self.skip_ch_s2, 64)    # Skip from features[2] output

        self.upconv1 = nn.ConvTranspose2d(64, 32, kernel_size=2, stride=2)
        # For the first skip connection, we can use the output of the initial conv layer (s0) or the first block (s1).
        # Using s0 (32 channels) might be more common as it's closer to the input resolution features.
        self.dec1 = self._conv_block(32 + self.skip_ch_s0, 32)    # Skip from features[0] output
        
        self.final_up = nn.ConvTranspose2d(32, 16, kernel_size=2, stride=2) # To original HxW
        self.final_conv = self._conv_block(16, 16)
    ...
    def forward(self, x):
        # Encoder - Accessing features as per EfficientNet structure
        # `encoder.features` is a Sequential module. We need its intermediate outputs.
        s0 = self.encoder.features[0](x)           # Initial Conv: H/2, 32 ch
        s1 = self.encoder.features[1](s0)          # Block 0: H/2, 16 ch
        s2 = self.encoder.features[2](s1)          # Block 1: H/4, 24 ch
        s3 = self.encoder.features[3](s2)          # Block 2: H/8, 40 ch
        # For s4, we need output of features[5]
        temp_s4 = self.encoder.features[4](s3)       # Block 3: H/16, 80 ch
        s4 = self.encoder.features[5](temp_s4)     # Block 4: H/16, 112 ch
        # For bottleneck, we need output of features[7]
        temp_bottleneck = self.encoder.features[6](s4) # Block 5: H/32, 192 ch
        bottleneck = self.encoder.features[7](temp_bottleneck) # Block 6: H/32, 320 ch
        
        # Decoder
        d4 = self.upconv4(bottleneck)          # H/16
        d4 = torch.cat((d4, s4), dim=1)        # Concat with s4 (112 ch)
        d4 = self.dec4(d4)

        d3 = self.upconv3(d4)                  # H/8
        d3 = torch.cat((d3, s3), dim=1)        # Concat with s3 (40 ch)
        d3 = self.dec3(d3)

        d2 = self.upconv2(d3)                  # H/4
        d2 = torch.cat((d2, s2), dim=1)        # Concat with s2 (24 ch)
        d2 = self.dec2(d2)

        d1 = self.upconv1(d2)                  # H/2
        d1 = torch.cat((d1, s0), dim=1)        # Concat with s0 (32 ch)
        d1 = self.dec1(d1)
        
        d0 = self.final_up(d1)                 # H
        d0 = self.final_conv(d0)

        out = self.conv_last(d0)               # H
        return out
...
class ModelTrainer:
    ...
    def _get_model(self):
        if self.config.MODEL_NAME == "EfficientNetB0UNet":
    ...
    def train(self):
        ...
        train_dataset_with_transform = BladderDataset(image_dir=self.config.TRAIN_IMG_DIR, mask_dir=self.config.TRAIN_MASK_DIR, transform=self.data_transforms.get_train_transforms())
        train_dataset = torch.utils.data.Subset(train_dataset_with_transform, train_indices.indices)

        val_dataset_with_transform = BladderDataset(image_dir=self.config.TRAIN_IMG_DIR, mask_dir=self.config.TRAIN_MASK_DIR, transform=val_transform)
        val_dataset = torch.utils.data.Subset(val_dataset_with_transform, val_indices.indices)
        ...
        if os.path.exists(best_model_path):
            ...
            # If training finished due to NUM_EPOCHS and no explicit best model was saved because the last epoch wasn't the best
            # We might want to load the state of the model at the end of training for visualization
            # Or, if early stopping happened, early_stopping.best_value would be from a previous epoch.
            # The current self.model state would be from the epoch it stopped at.
            final_epoch_str = f"epoch_{epoch+1}_final_state" if epoch + 1 < self.config.NUM_EPOCHS else f"epoch_{self.config.NUM_EPOCHS}_final_state"
...
def main():
    config = ModelConfig(
        MODEL_NAME="EfficientNetB0UNet",
        BATCH_SIZE=4, # EfficientNets can be memory intensive, adjust as needed
        LEARNING_RATE=1e-4,
        NUM_EPOCHS=100,
        ...
        SAVE_DIR="checkpoints_efficientnet_b0_unet", # Specific save directory
        FIGURE_DIR="figures_efficientnet_b0_unet"  # Specific figure directory
    )
