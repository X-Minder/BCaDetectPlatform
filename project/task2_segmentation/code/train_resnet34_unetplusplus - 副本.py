...
class ResNet34UNetPlusPlus(nn.Module):
    ...
    def forward(self, x):
        ...
        x4_0_orig_before_sa = self.encoder_x4_0(x3_0_orig)  # 先通过encoder
        x4_0_orig = self.self_attention_bottleneck(x4_0_orig_before_sa)  # 再通过Self-Attention
        # --- Apply Attention to Encoder Features ---
        # The gating signal 'g' for X_i_0 comes from an upsampled version of X_{i+1}
