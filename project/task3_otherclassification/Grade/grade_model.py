# %% [markdown]
# # Grade 膀胱镜图像二分类模型
# 通过迁移学习（EfficientNet-B0）对 Grade 进行二分类 (0/1)，按患者划分训练/验证/测试集。
...
from torchvision.models import efficientnet_b0, EfficientNet_B0_Weights, resnet34, ResNet34_Weights, swin_t, Swin_T_Weights, convnext_tiny, ConvNeXt_Tiny_Weights
    ...
    def map_grade_to_binary(grade_status): 
        ...
        try:
            ...
            if status_val != status_val_float: 
                 print(f"Warning: Grade status '{grade_status}' (str: '{status_str}') has non-zero decimal. Mapping to NaN.")
                 return np.nan

            if status_val in [0, 1]:
                return status_val
            else: 
                print(f"Warning: Unexpected Grade status '{grade_status}' (parsed as {status_val}). Not 0 or 1. Mapping to NaN.")
                return np.nan
        except ValueError:
            print(f"Warning: Could not convert Grade status '{grade_status}' (str: '{status_str}') to number. Mapping to NaN.")
            return np.nan

    label_df[MAPPED_LABEL_COLUMN_NAME] = label_df[LABEL_COLUMN_NAME].apply(map_grade_to_binary) 
    print(f"3. Rows after applying 'map_grade_to_binary' (before dropping NA from '{MAPPED_LABEL_COLUMN_NAME}'): {len(label_df)}")
...
# test_ds  = GradeDataset(df_test,  IMG_DIR, transform=val_tf) # Removed test_ds
...
# ## 模型定义与训练设置 for Grade
# %%
print(f"Using EfficientNet-B0 for {LABEL_COLUMN_NAME} {NUM_CLASSES}-class classification") 
weights = EfficientNet_B0_Weights.IMAGENET1K_V1
model = efficientnet_b0(weights=weights)

# EfficientNet-B0 的分类器是 model.classifier, 通常是 (Dropout, Linear)
# 我们需要替换其中的 nn.Linear 层
...
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', patience=8, factor=0.5, verbose=True, min_lr=1e-6)

# %% [markdown]
# ## 早停机制
# %%
class EarlyStopping:
    def __init__(self, patience=15, min_delta=0.0001, restore_best_weights=True, mode='min', verbose=True, task_name="Grade"): 
...
early_stopping = EarlyStopping(patience=15, mode='min', verbose=True, min_delta=0.0001, task_name=LABEL_COLUMN_NAME) 
...
for epoch in range(1, NUM_EPOCHS+1):
    model.train()
    running_loss = 0.0
    for imgs, labels in tqdm(train_loader, desc=f"Epoch {epoch}/{NUM_EPOCHS} [Train {LABEL_COLUMN_NAME}]"): 
    ...
    if len(val_ds) > 0:
        ...
        if val_epoch_loss < best_val_loss: 
            ...
            torch.save(model.state_dict(), f"best_model_{LABEL_COLUMN_NAME.lower()}.pth") 
            print(f"Epoch {epoch}: New best {LABEL_COLUMN_NAME} model saved with Val Loss: {best_val_loss:.4f}") 
        
        current_lr = optimizer.param_groups[0]['lr']
        history['lr'].append(current_lr)

        if early_stopping(val_epoch_loss, model):
            print(f"Early stopping triggered for {LABEL_COLUMN_NAME} based on validation loss.") 
            break
    else: 
...
# ## 绘制训练过程曲线 for Grade
# %%
def plot_training_history_grade(history): 
    ...
    fig.tight_layout()  
    plt.title(f'{LABEL_COLUMN_NAME} 二分类训练过程监控') 
    plt.xticks(epoch_ticks)
    plt.savefig(f"training_history_{LABEL_COLUMN_NAME.lower()}.png") 
    plt.show()

if any(history.values()):
    plot_training_history_grade(history) 
else:
    print(f"No training history to plot for {LABEL_COLUMN_NAME}.") 

# %% [markdown]
# ## 验证集最终评估曲线 (ROC & P-R) for Grade
...
def simulate_data_cleaning_test(model, original_val_loader, original_labels_np, num_samples_to_flip=100, random_state=SEED):
    ...
    # Create a deep copy of the validation dataset's underlying dataframe to modify labels
    df_val_copy = val_ds.df.copy()
    
    # Ensure indices are aligned if we sample from df_val_copy
    # However, it's simpler to flip labels in the numpy array and create a new dataset/loader if necessary
    # For this simulation, we'll just flip labels in the numpy array for AUC calculation.
    # A more rigorous test would retrain or re-evaluate on a modified DataLoader.
    ...
    # Evaluate model with these "corrected" labels
    # We need probabilities from the model, which we should already have from `all_val_probs_class1_final_np`
    # So, we don't need to re-run the model, just re-calculate AUC with flipped_labels_np
...
def perform_permutation_test(model, val_loader, original_labels_np, original_probs_np, n_permutations=1000):
    ...
    if len(np.unique(original_labels_np)) < 2:
        ...
        # We can still proceed, but AUC might be consistently 0.5 or error out for permuted labels.
    ...
    for i in tqdm(range(n_permutations), desc=f"Permutation Test {LABEL_COLUMN_NAME}"):
        ...
        if len(np.unique(permuted_labels)) < 2:
            # This can happen if original dataset is very small or extremely imbalanced
            # print(f"Permutation {i+1} resulted in single class. AUC is 0.5 or undefined.")
        ...
        try:
            ...
            permuted_aucs.append(auc)
             ...
             # print(f"ValueError for permuted labels in iter {i+1}. Appending 0.5")
             permuted_aucs.append(0.5) # Assign neutral AUC if calculation fails for a permutation
...
if 'model' in locals() and 'val_loader' in locals() and len(val_ds) > 0:
    ...
    # Ensure the best model is loaded for these analyses
    try:
        model.load_state_dict(torch.load(f"best_model_{LABEL_COLUMN_NAME.lower()}.pth"))
        ...
        # Need all_val_labels_final_np and all_val_probs_class1_final_np from previous validation step
        # If they don't exist, re-calculate them.
        if 'all_val_labels_final_np' not in globals() or 'all_val_probs_class1_final_np' not in globals() or \
            ...
            all_val_labels_final_list = []
            all_val_probs_class1_final_list = []
            model.eval()
            with torch.no_grad():
                for imgs, labels in tqdm(val_loader, desc=f"Final Validation for Analysis {LABEL_COLUMN_NAME}"):
                    ...
                    outputs = model(imgs)
                    probs = torch.softmax(outputs, dim=1)
                    all_val_probs_class1_final_list.extend(probs[:, 1].cpu().numpy())
                    all_val_labels_final_list.extend(labels.numpy())
            all_val_labels_final_np = np.array(all_val_labels_final_list)
            all_val_probs_class1_final_np = np.array(all_val_probs_class1_final_list)

        val_embeddings, val_true_labels_for_analysis = get_embeddings_convnext(model, val_loader, device)
        ...
        if val_embeddings is not None and len(val_embeddings) > 0 and len(val_true_labels_for_analysis) == len(val_embeddings):
            ...
            # Using model's probabilities for class 1 as a uni-dimensional feature for MI with true labels
            # Or use high-dimensional embeddings directly. Let's try with probabilities first as a simple check.
            # For a more direct feature-label check, use val_embeddings.
            # Let's average MI across embedding features if using val_embeddings.
            ...
            # Calculate MI using the full embeddings (might be slow/memory intensive if high-dim)
            # Taking mean of MI scores for each feature dimension for simplicity
            ...
            # This requires original_val_loader and original_labels_np (all_val_labels_final_np)
            # The function simulate_data_cleaning_test uses global `all_val_probs_class1_final_np` 
            # or recomputes if not available.
            simulate_data_cleaning_test(model, val_loader, all_val_labels_final_np, num_samples_to_flip=100)
            
            # 5. Perform Permutation Test
            # Requires all_val_labels_final_np and all_val_probs_class1_final_np
...
# %% [markdown]
# ## Grad-CAM 可视化 for Grade
# %%
GRADE_CLASS_NAMES = [f'{LABEL_COLUMN_NAME} Class 0 (Low)', f'{LABEL_COLUMN_NAME} Class 1 (High)'] # Re-define for Grad-CAM titles

def visualize_grad_cam_grade(model, dataset, device, num_images=4, target_classes_to_viz=None): 
    ...
    num_viz_cols = actual_num_images_per_class 
    ...
    if num_viz_rows * num_viz_cols == 1 : 
         axes = np.array([[axes[0]],[axes[1]]]).reshape(2,1,1) 
    elif num_viz_cols == 1: 
    ...
    if len(dataset) < num_viz_cols:
        ...
        num_viz_cols = len(dataset) 
        if num_viz_rows * num_viz_cols == 0: return 
        plt.close(fig) 
        fig, axes = plt.subplots(num_viz_rows * 2, num_viz_cols, figsize=(num_viz_cols * 3, num_viz_rows * 6))
    ...
    for r_idx, target_cls in enumerate(target_classes_to_viz):
        for c_idx_local, img_idx_in_dataset in enumerate(indices_to_use): 
            current_c_idx_for_plot = c_idx_local 
            ...
            title_str = f"""True: {GRADE_CLASS_NAMES[true_label]}
CAM for: {GRADE_CLASS_NAMES[target_cls]}""" 
...
if 'model' in locals() and 'val_ds' in locals() and len(val_ds) > 0:
    print(f"\nVisualizing Grad-CAM for {LABEL_COLUMN_NAME} model") 
    visualize_grad_cam_grade(model, dataset=val_ds, device=device, num_images=4, target_classes_to_viz=[0,1]) 
else:
    print(f"Skipping {LABEL_COLUMN_NAME} Grad-CAM: Model or test dataset not available or test_ds is empty.") 

# %%
print(f"{LABEL_COLUMN_NAME} binary classification model script generation complete.") 
print(f"IMPORTANT: Review and adjust 'LABEL_COLUMN_NAME' ('{LABEL_COLUMN_NAME}'), and the 'map_grade_to_binary' function if your Grade labels (0/1) need different parsing.") 
print(f"Also, review FocalLoss 'alpha' parameters and data loading for {LABEL_COLUMN_NAME} task.") 